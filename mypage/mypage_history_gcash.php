...
<?
	...
	//사용자 쿠폰함 내역 목록 불러오기
	$list = $cls_member->mypage_history_gcash_list($params, $total_cnt, $total_page);
                        ...
                        <table class="list">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th><?=getTransLangMsg("사용 내역")?></th>
                                    <th><?=getTransLangMsg("소진 G캐시")?></th>
                            ...
                            <tbody>
                                <?for ($i=0; $i<count($list); $i++) {?>
                                    <tr>
                                        ...
                                        <td><?=getGcashTypeName($list[$i]['gcash_type'])?></td>
                                        <td class="ta-l"><?=htmlDecode($list[$i]['content'])?></td>
                                        <td><?=formatNumbers(abs($list[$i]['gcash']))?> <?=getTransLangMsg("G캐시")?></td>
