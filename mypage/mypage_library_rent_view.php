<?
    ...
    $params['trans_idx']     = chkReqRpl("tidx", null, "", "", "INT");
    ...
    if (chkBlank($params['round_idx'])) fnMsgGo(503, getTransLangMsg("요청정보 값이 유효하지 않습니다."), "");
    if (!chkBlank($params['lang']) && array_search($params['lang'], array_column($lang_list, 'code')) === false) fnMsgGo(504, getTransLangMsg("요청정보 값이 유효하지 않습니다."), "");
    ...
    if (chkBlank($params['translator_id'])) {
        fnMsgGo(200, "", "/viewer/viewer.php?wt=". $params['webtoon_idx'] ."&ep=". $params['round_idx'], "");
    } else {
        fnMsgGo(200, "", "/viewer/viewer_trans.php?wt=". $params['webtoon_idx'] ."&ep=". $params['round_idx'] ."&tid=". $params['translator_id'] ."&tidx=". $params['trans_idx'], "");
    }
?>