...
<?
	...
	//번역자 정보로 접근시
	if (!chkBlank($params['translator_id']) || !chkBlank($params['translator_idx'])) fnMsgGo(500, getTransLangMsg("잘못된 접근 입니다."), "/", "");
	...
	if (isUser()) {
		//이용권 사용여부 체크
		$used_free_ticket = $cls_wt->freeticket_used_check($MEM_USR['usr_idx'], $wt_view['idx'], $wt_view['wait_free_time']);

		//웹툰 구매내역 목록 불러오기
		$purchase_history_list = $cls_wt->purchase_history_list($MEM_USR['usr_idx'], $wt_view['idx']);
			...
			if (!isUser()) {
				...
				if ($round_list[$i]['free_status'] == '10') {
					if ($wt_view['billing_type']=='20' && $round_list[$i]['free_exception']!='Y' && $used_free_ticket==false) {
						...
						$href    = "javascript:;";
						$onclick = "onclick=\"purchaseGo(". $round_list[$i]['idx'] .")\"";
					}
...
<script>
	...
	//웹툰 구매하기
	function purchaseGo(ep) {
		<?if (!isUser()) {?>
			popupNonLogin();
		<?} else {?>
			AJ.callAjax("/popup/webtoon_purchase.php", {"wt": "<?=$params['webtoon_idx']?>", "ep": ep}, function(data){
